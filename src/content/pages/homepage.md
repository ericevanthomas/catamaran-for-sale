---
title: Voyage 500 Catamaran For Sale
description: 50-foot 2010 Voyage 500 sailing catamaran for sale by owner, arriving St. Augustine, Florida June 1, 2026, for $379,000. Cruise-ready with extensive upgrades.
hero:
  eyebrow: 2010 Voyage 500 · S/V Tropicalia
  headline: Voyage 500 Catamaran For Sale by Owner
  headlineLines:
    - Voyage 500
    - Catamaran
    - For Sale
  subheadline: A 50-foot bluewater catamaran with a long waterline, fiberglassed bulkheads, an owner's hull, and a galley that cooked our family a full Thanksgiving dinner. Cruise-ready and lived-in.
  image: /images/hero-bow-edited.webp
  ctaText: Inquire About This Boat
  ctaLink: /contact/
glance:
  priceDisplay: $379,000 USD
  locationDisplay: Currently in the US Virgin Islands · Arriving St. Augustine, FL June 1, 2026
whyChose:
  eyebrow: From Our Family to Yours
  heading: Why we chose this boat.
  body: We spent over a year looking at catamarans before we found Tropicalia. She's sleeker and has a longer waterline than most of the newer production cats we considered, so she has less windage, a better motion through waves, and good bridgedeck clearance. The cockpit feels enclosed and safe with kids aboard. The galley is a real kitchen, not an afterthought. And the bulkheads and furniture are fiberglassed into the hull—not just glued—so nothing creaks or shifts. Her original owner was an aerospace engineer who specified her to a higher standard than the factory production spec, and she was the last 500 the Voyage factory ever built.
upgradesSection:
  eyebrow: Installed by Current Owners
  heading: Recent Upgrades
  items:
    - title: 1,280Ah LiFePO4 Battery Bank (2025)
      body: Four LiTime 12V 320Ah Mini Bluetooth lithium batteries (~16.4 kWh) wired in parallel. Professionally installed by marine electricians in St. Augustine with extra safety cutoffs. Each battery has a 200A BMS and a Bluetooth 5.0 app for real-time monitoring. Days of capacity under normal loads.
    - title: Rainman Modular AC-140 Watermaker (2025)
      body: Top-tier 220v AC unit—up to 37 GPH from two 40" Dow Filmtec membranes. Plumbed directly into the boat's freshwater system. Fill either tank individually; pickle in place via the unit's own reservoir.
    - title: Raymarine Axiom 12" Chartplotter (2025)
      body: New MFD with Navionics cartography for the US East Coast, Caribbean, and South America. Integrated with the Raymarine radar, AIS, and autopilot.
    - title: New Life Raft & AIS Safety Beacons
      body: Brand-new life raft, new EPIRB, stocked ditch bag, in-date flares, and AIS-enabled man-overboard beacons on all four life jackets—auto-deploy on water contact.
    - title: Victron Dual 110V/220V System
      body: 3000W 230V Victron inverter/charger (2024) + Multiplus 12/3000 120/50 (2022). Two shore inlets — plug in at any marina in the world.
    - title: Starlink Maritime
      body: Bimini-mounted, runs on 12V direct from the house bank. Full broadband at anchor, underway, or offshore — no generator required.
alternatingSections:
  - eyebrow: Value
    heading: A 50-foot bluewater catamaran with a new lithium bank, watermaker, chartplotter, and safety package.
    body: Tropicalia is a full-size, three-cabin owner's version with the range, systems, and space to cruise anywhere. She's been our family's home for four full cruising seasons, maintained every week and upgraded to the standard we wanted with our own kids aboard. Every major system has been replaced or refreshed in the last two years, and the maintenance trail is documented. You're stepping onto a cruiser with a known history and no deferred projects waiting for you.
    image: /images/home/home-why-value.webp
  - eyebrow: Comfort
    heading: Room to live, not just to sleep.
    body: At 27 feet of beam, the whole boat feels bigger than it measures. The entire starboard hull is the owner's suite — queen berth, private head, real storage. The salon and galley flow into the cockpit so meals, movie nights, and long evenings at anchor all happen in the same open space. The galley is a real kitchen — full-size oven, gas rotisserie, enough counter for two cooks — and the cockpit is enclosed and shaded. After four seasons aboard, it still feels like a home.
    image: /images/home/home-why-built.webp
  - eyebrow: Safety
    heading: Built solid and equipped for the ocean.
    body: "Tropicalia was built in South Africa with fiberglassed bulkheads tabbed into the hull — one solid structure that doesn't creak or shift underway. Her long waterline gives her an easy, predictable motion in a seaway. She's outfitted for real offshore passages: new life raft, new EPIRB, AIS man-overboard beacons on every life jacket, full Raymarine radar-chartplotter-autopilot suite, and redundant navigation. Every piece of safety gear has been checked, refreshed, or replaced this year."
    image: /images/home/home-why-ready.webp
highlights:
  eyebrow: Cruise-Ready
  heading: Key Equipment
  bullets:
    - 3-Cabin Owner's Version — entire starboard hull is the master
    - Extended rear deck — walk-around access with dinghy in chocks
    - Hot & cold showers in both sugar scoops
    - U-shaped galley with bar seating and a rotisserie oven
    - Washing machine + sheltered cockpit drying lines
    - Rainman Modular AC-140 watermaker (2026) — up to 37 GPH, plumbed in, pickle-in-place
    - New 1,280Ah lithium battery bank (2026) — professional install
    - Brand-new SD60 saildrives + rebuilt Brunton auto props (2025)
    - 800W solar, 4kW generator, Victron 110V + 220V dual inverters
    - Mabru air conditioning (12V + 110V)
    - New Raymarine Axiom 12" chartplotter + Orca nav display
    - AIS man-overboard beacons on every life jacket
    - Starlink satellite internet + Predict Wind position tracking
    - Zodiac 12.5' dinghy — 25hp 4-stroke, electric start, <100 hrs
    - Onboard dinghy-gas refill system from the starboard sugar scoop
    - USCG Documented & Duty Paid
bottomCta:
  headline: Buy Direct from the Owner
  body: No brokers, no commissions, no middlemen. You deal directly with the people who know this boat inside and out. We will answer every question honestly and make the process straightforward.
  primaryText: Get in Touch
  primaryLink: /contact/
  secondaryText: How Does a FSBO Sale Work?
  secondaryLink: /transaction/
---
