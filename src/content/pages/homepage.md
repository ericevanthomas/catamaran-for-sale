---
title: Voyage 500 Catamaran For Sale
description: 50-foot 2010 Voyage 500 sailing catamaran for sale by owner, arriving St. Augustine, Florida June 1, 2026, for $379,000. Cruise-ready with extensive upgrades.
hero:
  eyebrow: 2010 Voyage 500 · S/V Tropicalia
  headline: Voyage 500 Catamaran For Sale by Owner
  headlineLines:
    - Voyage 500
    - Catamaran
    - For Sale
  subheadline: A 50-foot bluewater catamaran with a long waterline, fiberglassed bulkheads, an owner's hull, and a galley that cooked our family a full Thanksgiving dinner.
  image: /images/hero-bow-edited.webp
  ctaText: Inquire About This Boat
  ctaLink: /contact/
glance:
  priceDisplay: $379,000 USD
  locationDisplay: Currently in the US Virgin Islands · Arriving St. Augustine, FL June 1, 2026
  items:
    - label: Year
      value: '2010'
    - label: Length
      value: 50 ft
    - label: Asking Price
      value: $379,000
    - label: Location
      value: US Virgin Islands · En route to St. Augustine, FL (06/01/2026)
    - label: Layout
      value: 3 Cabin Owner's Version
    - label: New Batteries
      value: 1,280Ah Lithium
    - label: Yanmar 3JH5e 39hp Engines
      value: 3,323 & 3,213 hours, new SD60 Sail Drives
    - label: New Equipment
      value: Watermaker, chartplotter, life raft, safety gear
  note: Recent upgrades include new Yanmar SD-60 saildrives, new lithium batteries (1,280Ah), new Rainman 37GPH watermaker, new Raymarine Axiom+ 12 MFD Chartplotter, new Raymarine Evolution EV-200 Sail Autopilot, new 6-person Offshore Commander life raft, new Orca Display and Orca Core, new ACR GlobalFix V5 AIS RLS EPIRB Survival Kit, new Dometic Masterflush electric head, 4 new German easyOne auto-deploy AIS offshore personal beacons, new Predictwind Hub, and other gear.
  linkText: Full Specs & Equipment
  linkUrl: /specs/
topPhotos:
  - image: /images/gallery/PXL_20260416_172152808.webp
    alt: Tropicalia at anchor, starboard profile
  - image: /images/gallery/PXL_20260419_203159996.RAW-01.COVER.webp
    alt: Interior of Voyage 500 Catamaran "Tropicalia"
  - image: /images/gallery/PXL_20260416_172251547.webp
    alt: Stern quarter view of Tropicalia at anchor
whyChose:
  eyebrow: From Our Family to Yours
  heading: Why we chose this boat.
  body: We spent nearly a year looking at catamarans before we found Tropicalia. She's sleeker and has a longer waterline than most of the newer production cats we considered. She has less windage, a better motion through waves, and good bridgedeck clearance. The cockpit feels enclosed and safe with kids aboard. The galley is a real kitchen, not an afterthought. And the bulkheads and furniture are fiberglassed into the hull—not just glued—so nothing creaks or shifts. The [Voyage Yachts factory](https://www.voyageyachts.com/) in South Africa custom built her for private owners in 2010, and she is the youngest 500 model in the fleet.
upgradesSection:
  eyebrow: ''
  heading: Recent Upgrades
  items:
    - title: 1,280Ah Lithium Battery Bank (2025)
      body: Four LiTime 12V 320Ah Bluetooth LiFePO4 batteries, professionally installed by marine electricians in St. Augustine with extra safety cutoffs. Each battery has a 200A BMS and a Bluetooth 5.0 app for real-time monitoring. Days of capacity under normal loads.
    - title: Rainman 37GPH Watermaker (2025)
      body: Top-tier 220v AC unit—up to 37 GPH from two 40" Dow Filmtec membranes. Plumbed directly into the boat's freshwater system. Fill either tank individually; pickle in place via the unit's own reservoir.
    - title: Raymarine Axiom 12" Chartplotter (2025)
      body: New MFD with Navionics cartography for the US East Coast, Caribbean, and South America. Integrated with the Raymarine radar, AIS, and autopilot. Professionally installed in St. Augustine.
    - title: New Life Raft & AIS Safety Beacons (2025)
      body: New 6-person offshore life raft, new EPIRB, stocked ditch bag, in-date flares, and AIS-enabled man-overboard beacons on all four life jackets—auto-deploy on water contact.
    - title: Victron Dual 110V/220V System
      body: 3000W 230V Victron inverter/charger (2024) + Multiplus 12/3000 120/50 (2022). Two shore inlets. Plug in at any marina in the world.
    - title: Starlink
      body: Pole-mounted, motor-actuated 110v unit.  Full broadband at anchor, underway, or offshore. 12V Starlink Mini new in box for backup.
alternatingSections:
  - eyebrow: Value
    heading: A 50-foot bluewater owner's version catamaran.
    body: Tropicalia is a three-cabin owner's version with the range and capacity to cruise anywhere. Fifty feet gives her real storage—water, food, fuel, tools, spares, dive equipment, and all the rest of your gear. Under sail she has a long waterline and a full-batten main on a Harken Battcar system. At anchor, 800W of solar, a 4kW Onan sound-insulated generator, Mabru air conditioning, a washing machine, and Starlink keep daily life easy.
    image: /images/gallery/PXL_20260419_214907153.RAW-01.COVER.webp
  - eyebrow: Comfort
    heading: Room to live your best cruising life.
    body: At 27 feet of beam, the main salon feels big, open, and airy. The salon and galley flow into the cockpit via sliding glass doors so meals, movie nights, and long evenings at anchor all happen in the same open space. The galley is a full kitchen with a proper gas cooktop and oven and oversize well-insulated freezer, and there is a 4-seat bar for casual meals and entertaining. The entire starboard hull is the owner's suite—king berth, private head with a large stall shower, office area, a walk-in closet, and real storage everywhere. The port hull features two queen cabins with private heads and a laundry area. Underway, the long waterline gives her an easy, level motion that lets everyone keep functioning in bigger seas. On deck, she carries a 12.5-foot Zodiac with a 25hp outboard, so shore runs and snorkel trips are quick and dry.
    image: /images/gallery/PXL_20260419_221625679.RAW-01.COVER.webp
  - eyebrow: Capability
    heading: Tropicalia is a well-equipped cruising machine.
    body: Tropicalia was custom built by Voyage Yachts in South Africa with bulkheads and furniture fiberglassed into the hull—one solid structure that doesn't creak or shift underway. She is energy- and water-independent, with 800 watts of solar panels, new 1,280ah lithium batteries, a 4kW Onan generator, a reliable new watermaker, and ample tankage. She has lots of cruising comforts, like a dedicated laundry room, an office, a giant two-door chest freezer, a separate refrigerator, and dual-zone Mabru air conditioning, including a 12V unit that can be run on the batteries, and all-electric winches. Dual 110v and 220v Victron chargers/inverters and dual shore power inlets mean she can cruise anywhere in the world. She carries a new 6-person offshore life raft, a new EPIRB, AIS man-overboard beacons on every life jacket, and a full Raymarine radar, Axiom chartplotter (2025), and autopilot suite (2025), for offshore navigation.
    image: /images/home/home-why-ready.webp
highlights:
  eyebrow: Cruise-Ready
  heading: Key Equipment
  bullets:
    - 3-Cabin Owner's Version — entire starboard hull is the master
    - Factory extended rear teak deck — walk-around access with dinghy in chocks
    - Hot & cold showers in both sugar scoops
    - U-shaped galley with bar seating and 3 deep sinks
    - Washing machine + sheltered cockpit drying lines
    - Rainman Modular AC-140 watermaker (2025) — 37 GPH
    - New 1,280Ah lithium battery bank (2025) — professionally installed
    - New Yanmar SD60 saildrives (2025) + Brunton Auto Props
    - 800W solar, 4kW generator, Victron 110V + 220V dual inverters
    - Mabru air conditioning (12V + 110V)
    - New Raymarine Axiom 12" chartplotter + Orca nav display + Orca Core
    - AIS man-overboard beacons on every life jacket
    - Starlink satellite internet + Predict Wind live position tracking
    - Zodiac 12.5' dinghy — 25hp 4-stroke Suzuki, electric start + trim, <100 hrs
    - Onboard dinghy-gas refill system from the starboard sugar scoop
    - USCG Documented & Duty Paid
bottomCta:
  headline: Buy Direct from the Owner
  body: You deal directly with the people who know this boat inside and out, without any markup for commission. Secure transaction, escrow, and USCG documentation transfer handled by a neutral third party.
  primaryText: Get in Touch
  primaryLink: /contact/
  secondaryText: How Does a FSBO Sale Work?
  secondaryLink: /transaction/
---
